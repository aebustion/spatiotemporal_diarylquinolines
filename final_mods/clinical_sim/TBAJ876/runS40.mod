;; 1. Based on: 0121
;; 2. Description: base model (three comp, linear elimination, F1 by fed/fasted, one transit cmpt) 
;; x1. Author: Connie 
$PROBLEM    TBAJ876 PK HUMAN
$INPUT      ID TIME DV EVID DOSE AMT II ADDL DIST
$DATA      ./20240502_TBAJ876_DIST_longer.csv IGNORE=@
$SUBROUTINE ADVAN6 TOL=5
$MODEL      NCOMP=12 COMP=(ABS,DEFDOSE) 
			COMP=(TRNS1) 
			COMP=(TRNS2)
            COMP=(CENTRAL,DEFOBS) 
            COMP=(PERIPHERAL1) 
...
$TABLE ID DOSE DIST TIME DV IPRED PRED CPP CPM AA11 AA12 EVID FILE=sdtabS40 NOPRINT ONEHEADER
