...
$DATA      ./20240502_TBAJ876_DIST.csv 
			IGNORE=@ 
			IGNORE(DIST.NE.300) ; only edge
			IGNORE(TIME.GT.672) ; only first month
$SUBROUTINE ADVAN6 TOL=5
$MODEL      NCOMP=12 
		COMP=(ABS,DEFDOSE)    ;1
		COMP=(TRNS1)          ;2
		COMP=(TRNS2)          ;3
            COMP=(CENTRAL,DEFOBS) ;4
            COMP=(PERIPHERAL1)    ;5
            COMP=(PERIPHERAL2)    ;6
            COMP=(METAB)           ;7
            COMP=(PERIPHERAL1META) ;8
            COMP=(PERIPHERAL2META) ;9
            COMP=(AUC)            ;10
            COMP=(PLESION)        ;11
            COMP=(MLESION)        ;12
$PK 
...
$TABLE ID DOSE DIST TIME DV IPRED PRED CPP CPM AA11 AA12 EVID FILE=sdtabS41 NOPRINT ONEHEADER
